...
1. The first method creates a base `theme` object based on Typography.js options or themes and uses a default, static `theme.styles` object to pick up values from the base theme.
2. The other method adds hard-coded styles to the `theme.styles` object without affecting the base theme object.

```js
import React from 'react'
import { ThemeProvider } from 'theme-ui'
import { toTheme, styles } from 'theme-ui-typography'
