...
While not being available in the `theme-ui` package directly, there are two separate entry points for React create element functions intended to be used in the [automatic JSX runtime](https://reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) with a custom pragma comment.
They add support for the `sx` prop, which uses Emotion's create element functions internally and parse style objects with the Theme UI `css` utility.
...
The `jsx` export is a React create element function for the classic JSX runtime intended for use with a custom pragma comment.
It adds support for the `sx` prop in the same way like the `/jsx-runtime` entrypoint.
