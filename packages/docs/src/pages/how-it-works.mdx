...
The Theme UI JSX pragma wraps the Emotion JSX pragma, converting the `sx` prop to a call to `@styled-system/css`. The two examples below yield the same results:
