...
you can take advantage of the _Variants_ feature.
...
If you want to combine multiple variants on the same element, pass an array to `variant` in the `sx` prop.
If styles conflict, those of the variant passed later will render.

```jsx
<button sx={{ variant: ['buttons.primary', 'buttons.small'] }} />
```

## Variant as a function

If you need to determine variants at run-time, pass a function to `variant` in the `sx` prop.

```jsx
<button
  sx={{
    variant: (theme: Theme) => {
      const buttonVariant = Object.keys(theme.buttons!)[3]
...
These variants can then be used in the `theme.styles` object, with the `sx` prop, or with the
[`Text`](/components/text) or [`Heading`](/components/heading) components.
...
- The entire [`styles` object](/theme-spec#styles) can style child HTML elements or Markdown content,
...
- If the [`useRootStyles` configuration option](/theming#configuration-flags) is enabled,
  the `styles.root` object will style the `<html>` element
