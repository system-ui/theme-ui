...
Under the hood, it uses a [custom pragma comment](/jsx-pragma) that converts a Styled System-aware `sx` prop into a style object and passes it to the Emotion's `jsx` function.
