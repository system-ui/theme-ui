...
The keys used for each scale come from this specification, and naming conventions for colors and typography are included
to help ensure deeper interoperability with components and modules from various other libraries and to help ensure that themes created with Theme UI are suitable for white-label-like applications.

## Theme Scales

The Theme UI `theme` object is made up of the following scales of values for use when styling UI.
...
In order to use Theme UI color modes, color scales should include *at least* a `text` and `background` color. This is used in the `ColorMode` component to set body foreground and background colors.
...
**Note:** This is *not* required, and plain objects will work as well.
...
Typically, these values should roughly map to heading levels, but is not required.
...
Theme UI currently does *not* have standards for defining combinations of typographic styles, but *variants* can be used for situations where it is useful to reuse combinations of these primitive values.
