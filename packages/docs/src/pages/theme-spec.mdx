...
In order to use Theme UI color modes, color scales should include _at least_ a `text` and `background` color.
This is used in the `ColorMode` component to set body foreground and background colors.
...
where the name may be anything, but typically `light` and `dark` should be used for applications with a dark mode.
...
Another common practice is to have a `buttons` object with variants like primary and secondary:
...
<!--
TODO: @styled-system/css variants are a slightly different implementation what the core
library does.

**Note**: Variants are implemented as a part of Styled System and are also covered in [its docs](https://styled-system.com/variants).
-->
...
Elements like `button` are not rendered by markdown, so they aren't included in the styles object,
consider using a [variant](/theme-spec#variants).
