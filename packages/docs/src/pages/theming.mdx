...
- Additionally, to apply styles to the `<html>` element instead, add the `config: { useRootStyles: true }` flag. It works the same way as and will override `useBodyStyles`.
