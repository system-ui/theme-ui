...
- To disable applying styles to the `<body>` element, add the `config.useBodyStyles: false` flag to your theme.
- Additionally, to apply styles to the `<html>` element instead, add the `config.useRootStyles` flag. It works the same way as and will override `config.useBodyStyles`.
