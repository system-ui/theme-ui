...
| `useRootStyles`            | `null || undefined` | Adds styles defined in `theme.styles.root` to the `<html>` element along with `color` and `background-color`         |
| `useBodyStyles`            | `true`              | Legacy: Adds styles defined in `theme.styles.root` to the `<body>` element along with `color` and `background-color` |
